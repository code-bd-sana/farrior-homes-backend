...
export class MailConsumer {
  ...
  async handleSendMail(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    try {
      this.logger.log(`Processing email to: ${to} (Attempt: ${retryCount + 1})`);
      ...
      if (retryCount < this.MAX_RETRIES) {
        this.logger.warn(`Failed to send email to ${to}. Retrying... (${retryCount + 1}/${this.MAX_RETRIES})`);
        ...
        this.logger.error(`Failed to send email to ${to} after ${this.MAX_RETRIES} attempts. Logging and dropping.`);
