...
export class PropertyService {
  ...
  private async deliverNotificationEmail(options: {
    ...
    try {
      ...
      console.log(`[Notification] ✅ Direct SMTP fallback sent for: ${options.to}`);
    }
  }
  ...
  private async notifyPropertyOwnerListingLive(property: any): Promise<void> {
    try {
      const ownerId = property?.propertyOwner?.toString?.() ?? String(property?.propertyOwner);
  ...
  private async notifySavedPropertyActivity(options: {
    ...
    try {
      ...
      console.error('[FavoritesActivity] Failed to notify saved-property users:', error);
    }
  }
  ...
  private async notifyAdminsMarketUpdates(options: {
    ...
    try {
      ...
      const [adminUsers, weeklyListings, monthlyListings, weeklyRevenueAgg, monthlyRevenueAgg] =
        await Promise.all([
          this.userModel
...
private async triggerPropertyTypeNotification(
  ...
  try {
    ...
    console.log(`[Notification] Property Name: ${savedProperty.propertyName}`);
    ...
    const totalSavedProperties = await this.savedPropertyModel.countDocuments();
    console.log(`[Notification] Total saved properties in DB: ${totalSavedProperties}`);
    ...
    console.log(`[Notification] All saved properties (without populate):`, 
    ...
    console.log(`[Notification] Finding saved properties with matching type: ${propertyType}`);
    ...
    console.log(`[Notification] After populate - found ${matchedSavedProperties.length} records`);
        ...
        matchedSavedProperties.flatMap((sp: any) => {
          if (sp.propertyId === null || !sp.userId) {
            if (sp.propertyId === null) {
              console.log(`[Notification] ❌ Skipping - propertyId is null (type mismatch)`);
            }
            if (!sp.userId) {
          ...
          if (isCreator) {
            ...
            console.log(`[Notification] ✅ Including user: ${userId} (saved property type: ${sp.propertyId.propertyType})`);
          }
    ...
    console.log(`[Notification] After filtering - unique users found: ${userIds.length}`);
    console.log(`[Notification] User IDs:`, userIds);

    if (userIds.length === 0) {
      console.log(`[Notification] ⚠️ No users found to notify for property type: ${propertyType}`);
      ...
      console.log(`[Notification] Total saved properties with type ${propertyType}: ${savedPropsWithThisType}`);
      return;
    }

    // Step 5: Fetch emails of matched users
    console.log(`[Notification] Fetching email addresses for ${userIds.length} users...`);
    ...
    users.forEach((user: any, index: number) => {
      console.log(`[Notification] User[${index}]: ID=${user._id}, Email=${user.email}, Name=${user.name || 'N/A'}`);
    });
    ...
    if (emails.length === 0) {
      console.log('[Notification] ❌ No valid emails found - all users missing email addresses');
      return;
    }
    ...
    const text = `New ${propertyType} Property Added!\n\n` +
    ...
    console.log(`[Notification] 📧 Attempting to send emails to ${limitedEmails.length} recipients...`);
    ...
    for (const email of limitedEmails) {
      try {
        ...
        results.push({ status: 'rejected', email, reason: error?.message || String(error) });
      }
    ...
    if (failed > 0) {
      console.log('Failed emails details:', results.filter(r => r.status === 'rejected'));
    }
    ...
    console.error('\n========== ❌ NOTIFICATION ERROR ==========');
    console.error('[Notification] Error in triggerPropertyTypeNotification:', err);
...
async update(
  ...
  console.log(`[Update] Update DTO:`, JSON.stringify(updatePropertyDto, null, 2));
  ...
  console.log(`[Update] Existing property - isPublished: ${existingProperty.isPublished}`);
  console.log(`[Update] Existing property - propertyType: ${existingProperty.propertyType}`);
  ...
  console.log(`[Update] Payload before update:`, JSON.stringify(payload, null, 2));
  ...
  const isExplicitlyPublishedNow =
    updatePropertyDto.isPublished === true || 
  ...
  const switchedToSale = previousStatus !== PropertyStatus.SALE && nextStatus === PropertyStatus.SALE;
  ...
  const shouldNotify =
    ...
    (!wasPreviouslyPublished && (isExplicitlyPublishedNow || isPublishedAfterUpdate)) ||
  ...
  const becameSold = previousStatus !== PropertyStatus.SOLD && nextStatus === PropertyStatus.SOLD;
  const becameRented = previousStatus !== PropertyStatus.RENT && nextStatus === PropertyStatus.RENT;
  const becameDelisted = existingProperty.isPublished === true && updated?.isPublished === false;


  if (shouldNotify && updated) {
    console.log(`[Update] ✅ Property is now published! Triggering notifications...`);
    console.log(`[Update] Property type for notification: ${updated.propertyType}`);
    
    // Fire & forget - with better error handling
    this.triggerPropertyTypeNotification(updated, user.userId)
      .then(() => {
        console.log(`[Update] ✅ Notification process completed successfully`);
      })
      .catch((err) => {
        console.error('[Update] ❌ Notification background pipeline failed:', err);
      });
  } else {
    console.log(`[Update] ❌ Not triggering notifications because:`);
    if (wasPreviouslyPublished) console.log(`[Update] - Property was already published`);
    if (!isExplicitlyPublishedNow && updated?.isPublished !== true)
      console.log(`[Update] - Property is not published`);
    if (!switchedToSale) console.log(`[Update] - Property status did not switch to sale`);
  }
  ...
  if (updated && becameDelisted) {
    this.notifySavedPropertyActivity({
      ...
      console.error('[Update] ❌ Favorites delisted notification failed:', err);
    });
  }
  ...
  if (updated) {
    ...
    console.log(`[Update] Signed URLs generated: ${imagesSignedUrls.length} images, thumbnail: ${!!thumbnailSignedUrl}`);
    ...
    return {
      ...
      propertyOwner:
        updated.propertyOwner?.toString?.() ??
        String(updated.propertyOwner),
      images: imagesSignedUrls,
      thumbnail: thumbnailSignedUrl,
    };
  }
  ...
  async remove(id: MongoIdDto['id'], user: AuthUser) {
    const property = await this.propertyModel
      .findById(id)
      .select('propertyOwner propertyName propertyType address isPublished status')
      .lean();
    ...
    if (deleted.deletedCount && property) {
      this.notifySavedPropertyActivity({
        ...
        console.error('[Remove] ❌ Favorites delisted notification failed:', err);
      });
    }
